    ...
    if (categoryActive == 0) {
      ...
      setProjectsFiltered(
        data
          ?.filter(
            (project) =>
              project.category == categories[categoryActive]
          )
          .sort(compare)
      );
    }
  }, [categoryActive, categories, data]);
...
function ProjectButton({
  ...
  return (
    ...
    >
      <motion.button
        className={`uppercase font-bold text-xl md:text-3xl flex relative h-full justify-center items-center px-4 transition duration-500 ${categoryActive == index
            ? "text-white"
...
function ProjectCard({ project, index }) {
  ...
  const colors = {
    personal: "#5183B4aa",
    practice: "#4DBD75aa",
    experiments: "#4DC0C4aa",
    recommended: "#e3c547aa",
  };
  ...
  return (
    <>
          ...
          >
            <SwiperSlide>
              <motion.div
                ...
                style={{
                  backgroundColor: colors[project.category],
                }}
              >
                <div className="rrelative rounded-3xl w-full h-full bg-[#f8efff] dark:bg-[#09030e] p-4 md:p-8 overflow-y-scroll">
                  ...
                  <p
                    layout
                    className={`text-lg ${(project.es?.description.length >
                        0 ||
                        project.en?.description.length >
                        0) &&
                      "h-1/3"
      ...
      <motion.div
        ...
        className={`relative col-span-3 md:col-span-1 aspect-square rounded-3xl overflow-hidden cursor-pointer p-4 pt-8 dark:text-white text-gray-50 shadow-xl ${showModal ? "opacity-0 -z-10" : "opacity-1"
          }`}
      ...
      >
        <motion.div
          className={`absolute bottom-0 overflow-hidden rounded-t-3xl transition-all duration-300 ease-in-out ${isHover
              ? "w-[95%] h-[55%] left-[2.5%]"
